...
/*		widths.sort(function(a, b){
	...
	'assignPages': function() {
	    ...
	    for ( i= 1; i <= pages ; i++) {
	        ...
	        if (i != pages) {
	            bcParent.find( '.' + pageClassName + i + ':last').addClass('last-item-in-page');
	        }
	...
	'hideAllCrumbs': function() {
		breadcrumbs.$crumbsCollection.addClass('hide');
	},

	'showPage': function($page) {
		breadcrumbs.$currentPage = parseInt($page);
		breadcrumbs.hideAllCrumbs();
		breadcrumbs.$parentEl.find(' > .' + breadcrumbs.$pageClass + $page).removeClass('hide');
	},

	'controlNav': function() {
		currPage = breadcrumbs.$currentPage;
		total = breadcrumbs.$totalPages;
		if( total > 1 ) {
			if( currPage == 1 ) {
				breadcrumbs.$navLeftEl.addClass('hide');
				breadcrumbs.$navRightEl.removeClass('hide');
			} else if ( currPage == total) {
				breadcrumbs.$navLeftEl.removeClass('hide');
				breadcrumbs.$navRightEl.addClass('hide');
			} else {
				breadcrumbs.$navLeftEl.removeClass('hide');
				breadcrumbs.$navRightEl.removeClass('hide');
			}
		} else {
			//keep the navs hidden
		}
	},

	'prevPage' : function() {
		if(breadcrumbs.$currentPage == 1) {
			//dont do anything
		} else if(breadcrumbs.$currentPage != 1 && breadcrumbs.$currentPage <= breadcrumbs.$totalPages) {
			breadcrumbs.$currentPage -= 1;
			breadcrumbs.showPage(breadcrumbs.$currentPage);
	...
	'nextPage' : function() {
		if(breadcrumbs.$currentPage == breadcrumbs.$totalPages) {
			//dont do anything
		} else if(breadcrumbs.$currentPage != breadcrumbs.$totalPages && breadcrumbs.$currentPage >= 1) {
			breadcrumbs.$currentPage += 1;
			breadcrumbs.showPage(breadcrumbs.$currentPage);
		}
